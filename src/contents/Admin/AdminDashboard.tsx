                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            <thead>
                                <tr style={{ borderBottom: "2px solid #f3f4f6", textAlign: "left" }}>
                                    ...
                                    <th style={{ padding: "12px 8px", fontSize: 13, color: TEXT_GRAY, fontWeight: 600 }}>상태</th>
                            ...
                            <tbody>
                                {recentUsers.map(user => (
                                    <tr key={user.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                                        <td style={{ padding: "16px 8px", fontSize: 14, color: TEXT_DARK, fontWeight: 500 }}>#{user.id}</td>
                                        <td style={{ padding: "16px 8px", fontSize: 14, color: TEXT_GRAY }}>{user.email}</td>
                                        <td style={{ padding: "16px 8px", fontSize: 14, color: TEXT_DARK }}>{user.name}</td>
                                        <td style={{ padding: "16px 8px" }}>
                                            <span style={{
                                                padding: "4px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600,
                                                backgroundColor: user.status === "활성" ? "#dcfce7" : user.status === "정지" ? "#fee2e2" : "#f3f4f6",
                                                color: user.status === "활성" ? "#16a34a" : user.status === "정지" ? "#dc2626" : TEXT_GRAY
                                            }}>
                                                {user.status}
                                            </span>
                                        </td>
                                        <td style={{ padding: "16px 8px", fontSize: 14, color: TEXT_GRAY }}>{user.date}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
